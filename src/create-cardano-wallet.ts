...
export async function createWallet(
  ...
  try {
    ...
    // Generate placeholder skey file (will contain reference to mnemonic)
    // MeshSDK doesn't expose the raw private key for security reasons
    // Users should use the mnemonic file for signing with our tools
    const skeyJson = {
      "type": "PaymentSigningKeyShelley_ed25519",
      "description": "Generated by Cardano Toolbox - Use mnemonic file for signing", 
      "note": "This wallet was generated with MeshSDK. Use the corresponding .mnemonic file for signing transactions.",
      "mnemonicFile": `${baseFilename}.mnemonic`
    };

    // Save skey reference file
    const skeyPath = join(WALLETS_DIR, `${baseFilename}.skey`);
    writeFileSync(skeyPath, JSON.stringify(skeyJson, null, 2));
    console.log(`✅ Signing Key Reference saved to: ${skeyPath}`);
    ...
    console.log(`  - Securely back up your mnemonic file ('${mnemonicPath}') and signing key ('${skeyPath}'). These can authorize transactions.`);
    console.log("  - Do NOT share your mnemonic or signing key with anyone.");
